...
---@class zTipOption :BackdropTemplate
...
--	self:Localize()
	...
	-- init frame
	self:SetWidth(300); self:SetHeight(680);
	...
	zTipOptionTitle:SetText(L["zTip Options"])

	self:CreateFontString("zTipOptionTips","ARTWORK","GameFontHighlight")
	zTipOptionTips:SetPoint("BOTTOM",0,10)
	zTipOptionTips:SetTextColor(1,0,1)
	zTipOptionTips:SetText("红色功能在某些情况下会导致报错")
	...
	zTipOptionPositionTitle:SetText(L["Positions"])
	...
	zTipOptionOffsetTitle:SetText(L["Offsets"])
	...
	zTipOptionOrigPosTitle:SetText(L["Original Position Offsets"])
...
function zTipOption:InitButtons()
	...
	self.Buttons = {
		...
		-- input box
		{name="OffsetX",type="EditBox",width=64,height=20,var="OffsetX",
			point="TOPLEFT",relative="OnCursorTop",rpoint="BOTTOMLEFT",y=-25},
		{name="OffsetY",type="EditBox",width=64,height=20,var="OffsetY",
			point="LEFT",relative="OffsetX",rpoint="RIGHT",x=105,},
		{name="OrigPosX",type="EditBox",width=64,height=20,var="OrigPosX",
			point="TOP",relative="OffsetX",rpoint="BOTTOM",y=-25,},
		{name="OrigPosY",type="EditBox",width=64,height=20,var="OrigPosY",
	...
	for key, value in ipairs(zTipOption.Buttons) do
		...
		if L[value.name] and L[value.name.."Tooltip"] then

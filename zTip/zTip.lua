...
function zTip:AddLine(line1, line2)
    if self.lastlinenum and self.lastlinenum < GameTooltip:NumLines() then
        ...
        _G["GameTooltipTextLeft" .. self.lastlinenum]:Show()
        if line2 then
            _G["GameTooltipTextRight" .. self.lastlinenum]:SetText(line2)
            _G["GameTooltipTextRight" .. self.lastlinenum]:Show()
        end
    else
        if line2 then
...
function DungeonsInTip(unit)
    if C_PlayerInfo and C_PlayerInfo.GetPlayerMythicPlusRatingSummary then
        if UnitIsPlayer(unit) and UnitLevel(unit) >= GetMaxLevelForPlayerExpansion() then
            ...
            if ratingSummary and ratingSummary.currentSeasonScore ~= 0 then
                local color = C_ChallengeMode.GetDungeonScoreRarityColor(ratingSummary.currentSeasonScore) or
                    HIGHLIGHT_FONT_COLOR
                if not RaiderIO then
                    zTip:AddLine("|cffffff00" ..
                        DUNGEON_SCORE_TOTAL_SCORE:format(color:WrapTextInColorCode(ratingSummary.currentSeasonScore)))
                end
                if IsControlKeyDown() then
                    if ratingSummary and ratingSummary.runs then
                        ...
                        for key, value in pairs(ratingSummary.runs) do
                            ...
                            local color = C_ChallengeMode.GetSpecificDungeonOverallScoreRarityColor(
        ...
        for i = 1, num do
            ...
            if (UnitIsUnit(unit1 .. "target", unit)) and (not UnitIsUnit(unit1, "player")) then
                ...
                if counter <= 5 then
                    ...
                    players = ("%s|cff%.2x%.2x%.2x%s|r, "):format(players, color.r * 255, color.g * 255, color.b * 255,
                        UnitName(unit1))
                end
                counter = (counter + 1)
            end
...
function zTip:GetVividMask()
    ...
    if mask.SetGradientAlpha then
        ...
        mask:SetGradient("VERTICAL", CreateColor(0, 0, 0, 0), CreateColor(1, 1, 1, 0.66))
    end
    mask:Hide()
...
function zTip:OnUpdate(elapsed)
    ...
    if elapsed then
        if UnitExists("mouseover") then
            ...
            -- 	GameTooltip:Hide()
            if zTip.unit and not zTipSaves.Fade and GameTooltip:IsOwned(UIParent) then
                if not zTip.lastX and not zTip.lastY then
                    zTip.lastX, zTip.lastY = GetCursorPosition()
                else
                    local lastX, lastY = GetCursorPosition()
                    if (zTip.lastX ~= lastX and zTip.lastY ~= lastY) then
                        zTip.lastX, zTip.lastY = nil, nil
                        GameTooltip:Hide()
                    end
                end
...
function zTip:Init()
    ...
    HInspect = Mixin(CreateFrame("FRAME", "zTip_HInspect"), HInspectMixin):Init(self.inspectDB, L["ItemLevel"], nil,
        HSetTooltip)
    --加载一些额外功能
    ...
    if TooltipDataProcessor then
        ...
        GameTooltip:HookScript(
            ...
            function(self, ...)
                zTip:OnTooltipSetUnit()
                -- zTip.NewSetDefaultAnchor_SetUnit(...)
            end
        )
    end
    GameTooltip:HookScript(
        ...
        function(self)
            ...
            -- GameTooltip:Hide() --关键,如果不隐藏,上次GameTooltip_SetDefaultAnchor为跟随,下次GameTooltip_SetDefaultAnchor为默认值,会出现错误,因为默认值
        end
    )
    ...
    hooksecurefunc(
        ...
        function(...)
            -- zTip.NewSetDefaultAnchor(...)
            zTip.SetDefaultAnchor(...)
        end
    )
...
zTip:Init() --提前加載,讓TooltipDataProcessor.AddTooltipPostCall(Enum.TooltipDataType.Unit,func)調用優先於其他插件


--factions
local name, standingId, isHeader, isCollapsed
function zTip:UpdatePlayerFaction()
    for i = 1, C_Reputation.GetNumFactions and C_Reputation.GetNumFactions() or GetNumFactions() do
        if C_Reputation.GetFactionDataByIndex then
            local data = C_Reputation.GetFactionDataByIndex(i)
            if data and not data.isHeader then
                self.factions[data.name] = data.factionID
            end
        else
...
--ANCHOR_CURSOR_RIGHT设定锚点,只给2个选择,左右
--TooltipDataProcessor.AddTooltipPostCall 用来更新当前状态
local test = "ANCHOR_CURSOR_RIGHT"
function zTip.NewSetDefaultAnchor(tooltip, parent)
    -- _G.print(parent:GetName())
    if InCombatLockdown() then
        -- if true then
        -- 	return
        -- end
        if parent.unit and UnitExists(parent.unit) then
            --框架目标单位
            -- _G.print(123)
            -- tooltip:SetOwner(parent, test, 5, 5)a
            ...
            if parent == UIParent then
                -- _G.print(34)
                --目标单位
                -- tooltip:SetOwner(parent, test, 5, 5)
            else
                -- _G.print(3564)
                --技能栏
                tooltip:SetOwner(parent, "ANCHOR_RIGHT")
    ...
    if name ~= tname then
        ...
        elseif UnitIsPlayer(unittarget) then
            ...
            if UnitIsEnemy(unittarget, "player") then
                ...
                -- white friend player
                tip = format("%s |cff%s%s|r |cffFFFFFF(%s)|r", tip, zTip:GetHexColor(RAID_CLASS_COLORS[(tmp2 or "")]),
...
function zTip:OnTooltipSetUnit()
    ...
    --~ 	BOSS战中隐藏
    if false and UnitExists("boss1") then
        GameTooltip:Hide()
        return
    end
...
--]]
    ...
    if levelline then
        ...
        elseif unitCreatureType then
            ...
            if unitCreatureType == L["NotSpecified"] and L["Specified"] then
            ...
            if zTipSaves.NPCClass then
                ...
                tmp2 = format("%s |cff%s%s|r", tmp2, zTip:GetHexColor(RAID_CLASS_COLORS[SYSCType]),
    ...
    if bplayer or bbattlepet then
        ...
        if not zTipSaves.DisplayPvPRank then
            GameTooltipTextLeft1:SetText(format("%s%s", tip, name or "NULL"))
        else
            local pvpname = UnitPVPName(unit)
            if not pvpname or pvpname == "" then
                pvpname = (name or "NULL")
            end
            GameTooltipTextLeft1:SetText(format("%s%s", tip, pvpname))
        end
    end
    ...
    if bplayer then
        ...
        if tip then
            if guild then
                ...
                -- local n = GameTooltipTextLeft1:GetText()
                -- GameTooltipTextLeft1:SetText( (n or "NONE").. " - " .. (tmp or ""))
                GameTooltipTextLeft1:SetText( (GameTooltipTextLeft1:GetText() or "").. " - " .. (tmp or ""))
            end
        end
    end
    ...
    if zTipSaves.ClassIcon then
        ...
        elseif unitrace and bplayer then
            ...
            cicon = zTip:GetClassIconForText(cls) or ""
        end
    end
    ...
    if bbattlepet then
        ...
        GameTooltipTextLeft1:SetText(cicon ..
            format("|cff%2x%2x%2x", r * 255, g * 255, b * 255) ..
            (GameTooltipTextLeft1:GetText() or "") .. "|r" .. (isafk and "<" .. AFK .. ">" or ""))
    end
    ...
    if tip or (levelline and levelline > 2) then
        ...
        elseif bbattlepet then
            ...
            GameTooltipTextLeft2:SetTextColor(r * zTip.GuildColorAlpha, g * zTip.GuildColorAlpha,
                b * zTip.GuildColorAlpha)
        end
    end
    --~ 标记本工会为亮色
...
function zTip:Slash(msg)
    ...
    elseif (param1 == "pp") then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cff00FFFFzTip:|r " .. format("%.2f", GetAddOnMemoryUsage("zTip")) .. " KB", 1, 1,
            0)
...
---- ID
local select, UnitBuff, UnitDebuff, UnitAura, tonumber, strfind, hooksecurefunc = select, UnitBuff, UnitDebuff, UnitAura,
    tonumber, strfind, hooksecurefunc
...
if TooltipDataProcessor then
    TooltipDataProcessor.AddTooltipPostCall(
        ...
        function(tooltip, tooltipData)
            if tooltipData.id then
                addLine(tooltip, tooltipData.id)
            end
        end
    )
    TooltipDataProcessor.AddTooltipPostCall(
        ...
        function(tooltip, tooltipData)
            if tooltipData.hasDynamicData and tooltipData.lines and tooltipData.lines[1] then
                -- printX(tooltipData.lines[1])
                if (tooltipData.lines[1].tooltipID) then
                    addLine(tooltip, tooltipData.lines[1].tooltipID)
                end
            end
        end
    )
    ...
    TooltipDataProcessor.AddTooltipPostCall(
        ...
        function(tooltip, tooltipData)
            if tooltipData.id then
                addLine(tooltip, tooltipData.id, true)
                -- local bag = GetItemCount(tooltipData.id)
                -- local bag_reagentbank = GetItemCount(tooltipData.id, false, false, true)
                -- local bag_bank_reagentbank = GetItemCount(tooltipData.id, true, false, true)
                -- local reagentbank = bag_reagentbank - bag
                -- local bank = bag_bank_reagentbank - bag_reagentbank
                -- local msg = bag_bank_reagentbank
                -- if bank > 0 or reagentbank > 0 then
                -- 	msg = msg .. "("
                -- 	if bank > 0 then
                -- 		msg = msg.."银行:"..bank..","
                -- 	end
                -- 	if reagentbank > 0 then
                -- 		msg = msg.."材料银行:"..reagentbank..","
                -- 	end
                -- 	msg = msg .. ")"
                -- end
                -- tooltip:AddDoubleLine("数量:", "|cffffffff" .. msg)
            end
        end
    )
else

...
local i, _
local GetTalentTabInfo, GetInventoryItemTexture, GetInventoryItemLink = GetTalentTabInfo, GetInventoryItemTexture,
...
function HInspectMixin:Init(cacheData, gear_Str, spec_Str, func)
    self.GearStr = gear_Str and (gear_Str .. CONST_SPL) or ("装等" .. CONST_SPL)
    if GameVer >= 60000 then
        self.TalentStr = spec_Str and (spec_Str .. CONST_SPL) or (SPECIALIZATION .. CONST_SPL)
    else
        self.TalentStr = spec_Str and (spec_Str .. CONST_SPL) or (TALENT .. CONST_SPL)
    end
...
function HInspectMixin:ScanInspectSpec(guid, info)
    ...
    elseif GameVer >= 20000 and GameVer < 40000 then
        ...
        for i = 1, numTalentGroups do
            msgTable[activeTalentGroup == i and 1 or 2] = HTool.TalentMsg(i,true)
        end
        info.Msg = TALENT_SPEC_PRIMARY .. CONST_SPL .. msgTable[1]
        if msgTable[2] then
            info.Msg = info.Msg .. "\r\n" .. TALENT_SPEC_SECONDARY .. CONST_SPL .. msgTable[2]
        end
    else
        local unit = HTool.GetUnit(guid)
        if not unit then
        ...
        if (unit == "player") then
            ...
            if specName then
                ...
                info.Msg = self.TalentStr ..
                    "|cff00ff00[" .. specName .. "]|r" .. "|T" .. icon .. ":12:12:0:0:10:10:0:10:0:10|t "
            else
...
function HInspectMixin:SetTooltipInspect(guid, flag)
    local user = self:GetInsInfo(guid)
    local gearstr = user.GearInfo.Msg and (user.GearInfo.Msg .. (user.GearInfo.IsFlag and "" or "*")) or
        user.GearInfo.StateInfo
    local specstr = user.SpecInfo.Msg and (user.SpecInfo.Msg .. (user.SpecInfo.IsFlag and "" or "*")) or
        user.SpecInfo.StateInfo

...
local function GetItemLevelFrame(self, category)
    ...
    if (TinyInspectClassicDB and TinyInspectClassicDB.EnableItemLevel) then
...
local function SetItemLevelString(self, text, quality, link)
    if (quality and TinyInspectClassicDB and TinyInspectClassicDB.ShowColoredItemLevelString) then
        local r, g, b, hex = GetItemQualityColor(quality)
        text = format("|c%s%s|r", hex, text)
    end
    --if (TinyInspectClassicDB and TinyInspectClassicDB.ShowCorruptedMark and link and IsCorruptedItem(link)) then
...
local function SetItemSlotString(self, class, equipSlot, link)
    local slotText = ""
    if (TinyInspectClassicDB and TinyInspectClassicDB.ShowItemSlotString) then
...
hooksecurefunc("SetItemButtonQuality", function(self, quality, itemIDOrLink, suppressOverlays, isBound)
    ...
    if (TinyInspectClassicDB and not TinyInspectClassicDB.EnableItemLevelOther) then
        return frame:Hide()
    end
...
LibEvent:attachEvent("ADDON_LOADED", function(self, addonName)
    if (addonName == "Blizzard_Communities" and GuildNewsButton_SetText) then
        ...
        hooksecurefunc("GuildNewsButton_SetText", function(button, text_color, text, text1, text2, ...)
            if (not TinyInspectClassicDB or 
                not TinyInspectClassicDB.EnableItemLevel or 
                not TinyInspectClassicDB.EnableItemLevelGuildNews) then
              return
            end
...
local function ChatItemLevel(Hyperlink)
    ...
    if (tonumber(level) and level > 0) then
        ...
        if (level) then
            local n, stats = 0, TI_GetItemStats(link)
...
local function filter(self, event, msg, ...)
    if (TinyInspectClassicDB and TinyInspectClassicDB.EnableItemLevelChat) then
...
LibEvent:attachTrigger("ITEMLEVEL_FRAME_SHOWN", function(self, frame, parent, category)
    if (TinyInspectClassicDB and not TinyInspectClassicDB["EnableItemLevel"..category]) then
        return frame:Hide()
    end
    if (TinyInspectClassicDB and TinyInspectClassicDB.PaperDollItemLevelOutsideString) then
        return
    end
    local anchorPoint = TinyInspectClassicDB and TinyInspectClassicDB.ItemLevelAnchorPoint

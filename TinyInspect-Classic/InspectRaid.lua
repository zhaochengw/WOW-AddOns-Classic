...
local function SendPlayerInfo()
    ...
    SendAddonMessage("TinyInspect-Classic", format("%s|%s|%s", "LV", ilvl, spec or ""), "RAID")
end

LibEvent:attachEvent("CHAT_MSG_ADDON", function(self, prefix, text, channel, sender)
    if (prefix == "TinyInspect-Classic" and channel == "RAID") then
...
LibEvent:attachEvent("GROUP_ROSTER_UPDATE", function(self)
	...
	--[[if (TinyInspectClassicDB and not TinyInspectClassicDB.EnableRaidItemLevel) then
		if (not IsInRaid()) then 
			if(IsInRaid() and not TinyInspectClassicDB.EnablePartyItemLevel) then
				return TinyInspectRaidFrame:Hide() 
			else
				return
			end
		else
			if(not TinyInspectClassicDB.EnablePartyItemLevel) then
				return TinyInspectRaidFrame:Hide() 
			end
		end
	end]]--
    ...
    if not cfg.EnableRaidItemLevel and not cfg.EnablePartyItemLevel then
    ...
    if IsInRaid() then
        ...
        -- => Raid option enabled => show frame and continue
        if f then f:Show() end

    else
        -- => In party but not raid
        -- => Party option disabled => hide frame and stop
        if not cfg.EnablePartyItemLevel then
            if f then f:Hide() end
            return
        end
        -- => Party option enabled => show frame and stop (party file continues)
        if f then f:Show() end
        return
    end

...
--边框类型下拉菜单
local OutlineTypeDropDown = {UFP_OP_None, UFP_OP_Outline_Thin, UFP_OP_Outline_Thick};
local function OutlineType_OnClick(self)
    UIDropDownMenu_SetSelectedID(UnitFramesPlus_OptionsFrame_OutlineType, self:GetID());
    UnitFramesPlusDB["target"]["outline"] = self:GetID() - 1;
    UnitFramesPlus_TargetExtraTextFontSize();
end

local function OutlineType_Init()
    local info;
    for id = 1, #OutlineTypeDropDown, 1 do
        info = {
            text = OutlineTypeDropDown[id];
            func = OutlineType_OnClick;
        };
        UIDropDownMenu_AddButton(info);
    end
end
...
if showOptions then
    ...
    -- 描边设置
    local OutlineText = UnitFramesPlus_Target_Options:CreateFontString(nil, "ARTWORK", "GameFontNormalSmall");
    OutlineText:ClearAllPoints();
    OutlineText:SetPoint("TOPLEFT", UnitFramesPlus_OptionsFrame_TargetExtraTextFontSizeSlider, "TOPLEFT", 300, 0);
    OutlineText:SetText(UFP_OP_Outline);

    local UnitFramesPlus_OptionsFrame_OutlineType = CreateFrame("Frame", "UnitFramesPlus_OptionsFrame_OutlineType", UnitFramesPlus_Target_Options, "UIDropDownMenuTemplate");
    UnitFramesPlus_OptionsFrame_OutlineType:ClearAllPoints();
    UnitFramesPlus_OptionsFrame_OutlineType:SetPoint("TOPLEFT", OutlineText, "TOPLEFT", -20, -10);
    UIDropDownMenu_SetWidth(UnitFramesPlus_OptionsFrame_OutlineType, 95);
    UIDropDownMenu_Initialize(UnitFramesPlus_OptionsFrame_OutlineType, OutlineType_Init);
    UIDropDownMenu_SetSelectedID(UnitFramesPlus_OptionsFrame_OutlineType, (UnitFramesPlusDB["target"]["outline"] or 0) + 1);

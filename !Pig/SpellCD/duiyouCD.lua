...
---ADD
fuFrame.list.E = CreateFrame('EditBox', nil, fuFrame.list,"BackdropTemplate");
fuFrame.list.E:SetSize(90,30);
fuFrame.list.E:SetBackdrop({ bgFile = "interface/common/common-input-border.blp",insets = {left = -6,right = 0,top = 2,bottom = -13}})
...
fuFrame.list.zairuMoren:SetScript("OnClick", function (self)
	...
	for i=1,#Spell_moren do
		local name, rank, icon, castTime, minRange, maxRange, spellId = GetSpellInfo(Spell_moren[i])
		local cooldownMS, gcdMS = GetSpellBaseCooldown(Spell_moren[i])
		local link="\124cff71d5ff\124Hspell:"..Spell_moren[i].."\124h["..name.."]\124h\124r";
		table.insert(PIG_Per['SpellJK']["Spell_list"][tabID], {spellId,name,icon,link,cooldownMS});
	end
	gengxinhang(fuFrame.list.Scroll)
end)
...
fuFrame:SetScript("OnShow", function()
	...
	fuFrame.JindutiaoH_Slider:SetValue(PIG_Per['SpellJK']["WHF_list"][tabID]["H"]);
	fuFrame.JindutiaoH_Slider.Text:SetText(PIG_Per['SpellJK']["WHF_list"][tabID]["H"]);
	fuFrame.JindutiaoFont_Slider:SetValue(PIG_Per['SpellJK']["WHF_list"][tabID]["font"]);
	fuFrame.JindutiaoFont_Slider.Text:SetText(PIG_Per['SpellJK']["WHF_list"][tabID]["font"]);
end)
--========================================================

...
--===================================
local function add_FmRdUI()
	if daiben_UI then return end
	...
	local function TimeRecord()
		...
		if instanceType=="party" or instanceType=="raid" and maxPlayers~=40 then
			...
			daiben_UI:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
			daiben_UI:RegisterEvent("CHAT_MSG_COMBAT_XP_GAIN");
			daiben_UI:RegisterEvent("CHAT_MSG_COMBAT_FACTION_CHANGE");
		...
		if instanceType=="none" then
			daiben_UI:UnregisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
			daiben_UI:UnregisterEvent("CHAT_MSG_COMBAT_XP_GAIN");
			daiben_UI:UnregisterEvent("CHAT_MSG_COMBAT_FACTION_CHANGE");
	...
	--daiben_UI:RegisterEvent("COMBAT_LOG_EVENT_UNFILTERED");
	--daiben_UI:RegisterEvent("CHAT_MSG_SAY");
	daiben_UI:RegisterEvent("CHAT_MSG_COMBAT_FACTION_CHANGE");
	daiben_UI:SetScript("OnEvent",function (self,event,arg1,arg2,_,_,arg5,_,_,_,_,_,_,arg12)
	...
	---
	addonTable.ADD_shezhi_F_UI()
	---
	local name, instanceType = GetInstanceInfo()
	if instanceType=="none" then
		if PIG_Per["FarmRecord"]["YijingCZ"] == "ON" then
			daiben_UI.CZbutton:SetText(jiuweiTXT)
		elseif PIG_Per["FarmRecord"]["YijingCZ"] == "OFF" then
			daiben_UI.CZbutton:SetText(chongzhiTXT)
		end
	else
		daiben_UI.CZbutton:SetText(jiuweiTXT)
	end
	...
	if PIG_Per["FarmRecord"]["autohuifu"]=="ON" then
		daiben_UI.yesno:SetChecked(true);
		daiben_UI.yesno.Tex:SetTexture("interface/common/indicator-green.blp");
	...
	daiben_UI:RegisterEvent("PLAYER_LOGOUT");
	daiben_UI:RegisterEvent("PLAYER_ENTERING_WORLD");
	daiben_UI:RegisterEvent("PLAYER_LEAVING_WORLD");
	daiben_UI:RegisterEvent("CHAT_MSG_SYSTEM");
	daiben_UI:RegisterEvent("CHAT_MSG_PARTY_LEADER");
	daiben_UI:RegisterEvent("GROUP_ROSTER_UPDATE");
	daiben_UI:RegisterEvent("PLAYER_MONEY");
	daiben_UI:RegisterEvent("CHAT_MSG_WHISPER");
	daiben_UI:RegisterEvent("TRADE_SHOW");
	daiben_UI:RegisterEvent("TRADE_PLAYER_ITEM_CHANGED");
	daiben_UI:RegisterEvent("TRADE_TARGET_ITEM_CHANGED");
	daiben_UI:RegisterEvent("TRADE_MONEY_CHANGED");
	daiben_UI:RegisterEvent("TRADE_ACCEPT_UPDATE");
	daiben_UI:RegisterEvent("UI_INFO_MESSAGE");
end
---===========================================================================
			...
			daibenzhushou_but:SetScript("OnClick", function(event, button)
				if daiben_UI:IsShown() then
					...
					if PIG_Per["FarmRecord"]["MaxMin"]=="ON" then
						...
						daiben_UI.Max_Min:SetNormalTexture("interface/chatframe/ui-chaticon-maximize-up.blp");
						daiben_UI.Max_Min:SetPushedTexture("interface/chatframe/ui-chaticon-maximize-down.blp")
					end
				end
			end);
		end

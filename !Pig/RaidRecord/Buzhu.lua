...
fuFrame.T_text1:SetText("\124cffFFFF00坦克/开怪猎人\124r");
...
fuFrame.T_text1_tishi:SetSize(30,30);
fuFrame.T_text1_tishi:SetPoint("RIGHT",fuFrame.T_text1,"LEFT",5,-1);
...
fuFrame.T_text1_tishi.Texture:SetAllPoints(fuFrame.T_text1_tishi)
fuFrame.T_text1_tishi:SetScript("OnEnter", function (self)
	GameTooltip:ClearLines();
	GameTooltip:SetOwner(self, "ANCHOR_TOPLEFT",0,0);
	GameTooltip:AddLine("提示：")
	GameTooltip:AddLine("\124cff00ff00在人员信息界面设置坦克补助/开怪猎人的玩家会显示在此处。\n如需修改请到人员信息界面修改。\124r")
	GameTooltip:Show();
end);
...
fuFrame.T_E = CreateFrame('EditBox', nil, fuFrame, "BackdropTemplate");
fuFrame.T_E:SetSize(48,33);
fuFrame.T_E:SetBackdrop({ bgFile = "interface/common/common-input-border.blp",insets = {left = -6,right = 0,top = 2,bottom = -13}})
    ...
    for i=1, 8 do
    	if #PIG["RaidRecord"]["Raidinfo"][i]>0 then
			for ii=1, #PIG["RaidRecord"]["Raidinfo"][i] do
				if PIG["RaidRecord"]["Raidinfo"][i][ii][5]=="坦克补助" or PIG["RaidRecord"]["Raidinfo"][i][ii][5]=="开怪猎人" then
	...
	if #Trenyuan>0 then
		...
		for i = 1, hang_NUM do
			...
			if Trenyuan[dangqian] then
				_G["buzhu_T_hang_"..i]:Show();
				_G["buzhu_T_hang_"..i].Tex:SetTexCoord(unpack(Trenyuan[dangqian][1]));
				_G["buzhu_T_hang_"..i].name:SetText(Trenyuan[dangqian][4]);
				_G["buzhu_T_hang_"..i].name:SetTextColor(Trenyuan[dangqian][2][1], Trenyuan[dangqian][2][2], Trenyuan[dangqian][2][3], 1);
				_G["buzhu_T_hang_"..i].G.V:SetText(Trenyuan[dangqian][6]);
			end
	    end
	end
...
fuFrame.bobaobuzhu_T:SetScript("OnEnter", function ()
	GameTooltip:ClearLines();
	GameTooltip:SetOwner(fuFrame.bobaobuzhu_T, "ANCHOR_TOPLEFT",20,0);
...
fuFrame.bobaobuzhu_T:SetScript("OnMouseDown", function ()
	fuFrame.bobaobuzhu_T.Tex:SetPoint("CENTER",2.5,-1.5);
end);
fuFrame.bobaobuzhu_T:SetScript("OnMouseUp", function ()
	fuFrame.bobaobuzhu_T.Tex:SetPoint("CENTER",4,0);
end);
fuFrame.bobaobuzhu_T:SetScript("OnClick", function()
	...
	for x=1,#PIG["RaidRecord"]["Raidinfo"] do
		for xx=1,#PIG["RaidRecord"]["Raidinfo"][x] do
			if PIG["RaidRecord"]["Raidinfo"][x][xx][5]=="开怪猎人" then
				SendChatMessage("["..PIG["RaidRecord"]["Raidinfo"][x][xx][5].."]-"..PIG["RaidRecord"]["Raidinfo"][x][xx][4].."-支出："..PIG["RaidRecord"]["Raidinfo"][x][xx][6].."G", xuanzhongName, nil);
			end
		end
	end
...
for id = 1, hang_NUM do
	...
	buzhu_T_hang.Tex:SetTexture("Interface/TargetingFrame/UI-Classes-Circles");
	...
	buzhu_T_hang.G.E = CreateFrame('EditBox', nil, buzhu_T_hang.G, "BackdropTemplate");
	buzhu_T_hang.G.E:SetSize(54,hang_Height);
	buzhu_T_hang.G.E:SetBackdrop({ bgFile = "interface/common/common-input-border.blp",insets = {left = -6,right = 0,top = 2,bottom = -13}})
...
fuFrame.N_text1_tishi:SetSize(30,30);
fuFrame.N_text1_tishi:SetPoint("RIGHT",fuFrame.N_text1,"LEFT",5,-1);
...
fuFrame.N_text1_tishi.Texture:SetAllPoints(fuFrame.N_text1_tishi)
...
fuFrame.N_E = CreateFrame('EditBox', nil, fuFrame, "BackdropTemplate");
fuFrame.N_E:SetSize(48,33);
fuFrame.N_E:SetBackdrop({ bgFile = "interface/common/common-input-border.blp",insets = {left = -6,right = 0,top = 2,bottom = -13}})
	...
	if #Nrenyuan>0 then
		...
		for i = 1, hang_NUM do
			...
			if Nrenyuan[dangqian] then
				_G["buzhu_N_hang_"..i]:Show();
				_G["buzhu_N_hang_"..i].Tex:SetTexCoord(unpack(Nrenyuan[dangqian][1]));
				_G["buzhu_N_hang_"..i].name:SetText(Nrenyuan[dangqian][4]);
				_G["buzhu_N_hang_"..i].name:SetTextColor(Nrenyuan[dangqian][2][1], Nrenyuan[dangqian][2][2], Nrenyuan[dangqian][2][3], 1);
				_G["buzhu_N_hang_"..i].G.V:SetText(Nrenyuan[dangqian][6]);
			end
	    end
	end
	addonTable.RaidRecord_UpdateG();
...
for id = 1, hang_NUM do
	...
	buzhu_N_hang.Tex:SetTexture("Interface/TargetingFrame/UI-Classes-Circles");
	...
	buzhu_N_hang.G.E = CreateFrame('EditBox', nil, buzhu_N_hang.G, "BackdropTemplate");
	buzhu_N_hang.G.E:SetSize(54,hang_Height);
	buzhu_N_hang.G.E:SetBackdrop({ bgFile = "interface/common/common-input-border.blp",insets = {left = -6,right = 0,top = 2,bottom = -13}})

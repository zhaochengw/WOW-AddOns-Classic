...
local function jisuanfenGData()
		...
		for xx=1,#PIG["RaidRecord"]["Raidinfo"][x] do
			if #PIG["RaidRecord"]["Raidinfo"][x][xx]>0 then
				...
				elseif PIG["RaidRecord"]["Raidinfo"][x][xx][5]=="治疗补助" then
					...
					_G["duiwu_"..x.."_"..xx].buzhu:SetWidth(duiwu_Height-8);
				elseif PIG["RaidRecord"]["Raidinfo"][x][xx][5]=="开怪猎人" then
	...
	local jingshouru=tonumber(Jing_RS_V_UI:GetText());
	...
	for x=1,8 do
		...
		for xx=1,#PIG["RaidRecord"]["Raidinfo"][x] do
			...
			if #PIG["RaidRecord"]["Raidinfo"][x][xx]>0 then	
				if PIG["RaidRecord"]["Raidinfo"][x][xx][5]=="坦克补助" or PIG["RaidRecord"]["Raidinfo"][x][xx][5]=="治疗补助" or PIG["RaidRecord"]["Raidinfo"][x][xx][5]=="开怪猎人" then
...
fenG.renshu_buzhu_LR_t:SetText("|cff00ff00开怪:|r")
...
fenG.guangbo:SetScript("OnClick", function ()
	...
	for id=1,#PIG["RaidRecord"]["ItemList"] do
		if PIG["RaidRecord"]["ItemList"][id][9]>0 or PIG["RaidRecord"]["ItemList"][id][14]>0 then
			if PIG["RaidRecord"]["ItemList"][id][14]>0 then
				SendChatMessage(PIG["RaidRecord"]["ItemList"][id][2].."收入："..PIG["RaidRecord"]["ItemList"][id][9]+PIG["RaidRecord"]["ItemList"][id][14].."G(买方<"..PIG["RaidRecord"]["ItemList"][id][8]..">尚欠"..PIG["RaidRecord"]["ItemList"][id][14]..")", xuanzhongName, nil);
			else
				SendChatMessage(PIG["RaidRecord"]["ItemList"][id][2].."收入："..PIG["RaidRecord"]["ItemList"][id][9].."G(买方<"..PIG["RaidRecord"]["ItemList"][id][8]..">)", xuanzhongName, nil);
			end
		else
			if PIG["RaidRecord"]["Rsetting"]["liupaibobao"]=="ON" then
				table.insert(liupaichupin,PIG["RaidRecord"]["ItemList"][id][2]);
			end
		end
	end
	...
	local Wupin_SR=Wupin_SR_V_UI:GetText();
	local fakuan_SR=fakuan_SR_V_UI:GetText();
	local buzhu_ZC=buzhu_SR_V_UI:GetText();
	local jiangli_ZC=jiangli_SR_V_UI:GetText();
	local ZongSR=ZongSR_V_UI:GetText();
	local Jing_RS=Jing_RS_V_UI:GetText();
	...
	SendChatMessage("=====!Pig开团助手=====", xuanzhongName, nil);
end);
...
----==============================================================
xiafangFrame_UI.fenG_BUT = CreateFrame("Button","fenG_BUT_UI",xiafangFrame_UI, "UIPanelButtonTemplate");  
xiafangFrame_UI.fenG_BUT:SetSize(80,28);
xiafangFrame_UI.fenG_BUT:SetPoint("LEFT",huoquRaidInfo_UI,"RIGHT",38,0);
xiafangFrame_UI.fenG_BUT:SetText("分G助手");
xiafangFrame_UI.fenG_BUT:SetMotionScriptsWhileDisabled(true)
xiafangFrame_UI.fenG_BUT:SetScript("OnEnter", function (self)
...
xiafangFrame_UI.fenG_BUT:SetScript("OnLeave", function ()
	GameTooltip:ClearLines();
	GameTooltip:Hide() 
end);
xiafangFrame_UI.fenG_BUT:SetScript("OnClick", function ()
	History_UI:Hide();
	RsettingF_UI:Hide();
	invite_UI:Hide();

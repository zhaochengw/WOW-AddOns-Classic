		...
		if ( i ~= 2 ) then
			...
			chatID.AddMessage = function(frame, text, ...)
				if PIG['ChatFrame']['jingjian']=="ON" then
					...
					local msninfoYES2=text:find("PIG", 1)
					if msninfoYES then
					...
					elseif msninfoYES2 then
						local Guolvweimian=text:find("请求位面信息", 1)
						local GuolvChedui=text:find("请求车队信息", 1)
						if Guolvweimian or GuolvChedui then
							return msninfo(frame, "|cff222222"..text.."|r", ...)
						else
							return msninfo(frame, "|cff66ffcc"..text.."|r", ...)
						end
					else
...
---===========================================================================
--自动加入寻求组队频道
local function JoinPermanentChannel456()
	--local pindaolist ={"公会招募"};
	local pindaolist ={"世界防务","寻求组队"};
	for i=1,#pindaolist do
		local channel,channelName= GetChannelName(pindaolist[i])
		if channelName==nil then
			JoinPermanentChannel(pindaolist[i], nil, DEFAULT_CHAT_FRAME:GetID(), 1);
			ChatFrame_AddChannel(DEFAULT_CHAT_FRAME, pindaolist[i])--订购一个聊天框以显示先前加入的聊天频道
		end
...
fuFrame.JoinPindao.Text:SetText("自动加入寻求组队频道");
fuFrame.JoinPindao.tooltip = "进入游戏后自动加入寻求组队/世界防务频道！";
fuFrame.JoinPindao:SetScript("OnClick", function (self)
	if self:GetChecked() then
		PIG['ChatFrame']['JoinPindao']="ON";
		JoinPermanentChannel456();
	else
...
addonTable.ChatFrame_QuickChat = function()
	...
	if PIG['ChatFrame']['JoinPindao']=="ON" then
		...
		C_Timer.After(3, JoinPermanentChannel456);
	end
	C_Timer.After(3, JoinPermanentPIG);
end
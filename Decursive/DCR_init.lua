--[[
    ...
    Decursive (v 2.7.8.8-1-gf03c614) add-on for World of Warcraft UI
...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "f03c614"; -- not used here but some other add-on may request it from outside
    D.date = "2022-09-14T10:38:02Z";
    D.version = "2.7.8.8-1-gf03c614";

    if D.date ~= "@project".."-date-iso@" then
        -- 1663151882 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.7.8.8-1-gf03c614|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.7.8.8-1-gf03c614"):lower()):find("beta") or ("2.7.8.8-1-gf03c614"):find("RC") or ("2.7.8.8-1-gf03c614"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.7.8.8-1-gf03c614" then
            self.db.global.NonRelease = "2.7.8.8-1-gf03c614";
...
T._LoadedFiles["DCR_init.lua"] = "2.7.8.8-1-gf03c614";
...
150
    ...
    Note: does not work for git
f03c614e8b0a3e994dca116532217b31012515a9

...
--10行 字体变大范围从原来的12-42,调整成12-100.Fixed by 黑山老鬼/部落大人2024.6.28
SetSliderData("DEX_Font", 1, 4)
SetSliderData("DEX_FontSize", 12, 100)
...
function DEX_RefreshCheckButton(name)
    ...
    if not button then
        DEFAULT_CHAT_FRAME:AddMessage("DamageEx: Could not find button "..tostring(name));
        return;
    end
    ...
    if DEXOptionsFrameCheckButtons[name] then

...
function TradeLogFrame_UpdateItem(frame, item)
    if (item) then
        TradeLogFrame_UpdateItemDetail(frame, item.name, item.texture, item.numItems, item.isUsable, item.enchantment,
            item.itemLink, item.quality);
    else
        TradeLogFrame_UpdateItemDetail(frame);
    end
end

function TradeLogFrame_UpdateItemDetail(frame, name, texture, numItems, isUsable, enchantment, itemLink, quality)
    ...
    if (id == 7) then
        ...
        if not quality then
            quality = itemLink and select(3, GetItemInfo(itemLink))
        end
        if quality then
    ...
    --SetItemButtonQuality(tradeItemButton, quality, itemLink)
end
...
function TradeListScrollFrame_Update(self)
    ...
    line = 1
    while line <= 15 do
        local length = table.getn(TradeLog_TradesHistory)
        if offset <= length then
            local trade = TradeLog_DB.ListDescending and TradeLog_TradesHistory[length - offset + 1] or TradeLog_TradesHistory[offset]
            --local trade = TradeLog_TradesHistory[length - offset + 1];
            if trade and (not TradeListOnlyCompleteCB:GetChecked() or trade.result == "complete") then
                local _, _, month, day, hour, min = string.find(trade.when, "(%d+)-(%d+) (%d+):(%d+)")
                getglobal("TradeListFrameButton" .. line).offset = offset;

                getglobal("TradeListFrameButton" .. line .. "Time"):SetText(month ..
                TRADE_LOG_MONTH_SUFFIX .. day .. TRADE_LOG_DAY_SUFFIX .. " " .. hour .. ":" .. min);
...
function TradeListFrame_OnLoad(self)
    Mixin(self, BackdropTemplateMixin)
    self:OnBackdropLoaded()
    self:HookScript("OnSizeChanged", self.OnBackdropSizeChanged)
    self:SetBackdrop({

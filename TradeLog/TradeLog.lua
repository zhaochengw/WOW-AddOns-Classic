...
TradeLog_DB = {
	...
	OnlyCompleteChecked = false,
	LogFrameScale = 1.3,
	ListDescending = false
}
...
function TradeLog_OnLoad(self)
	...
	cb:SetScript("OnClick", function(self) TradeLog_DB.AnnounceChecked = self:GetChecked() and true or false; end);
...
function TradeLog_OnEvent(self, event, arg1, arg2, ...)
	...
	elseif (event == "VARIABLES_LOADED") then
		...
		TradeLogFrameScaleSlider:SetValue(TradeLog_DB.LogFrameScale)
		TradeListOnlyCompleteCB:SetChecked(TradeLog_DB.OnlyCompleteChecked)
		TradeListDescending:SetChecked(TradeLog_DB.ListDescending)

		UIDropDownMenu_Initialize(TBT_AnnounceChannelDropDown, TBT_AnnounceChannelDropDown_Initialize);
		UIDropDownMenu_SetSelectedValue(TBT_AnnounceChannelDropDown, TradeLog_DB.AnnounceChannel);

		if TradeLog_DB.AnnounceChecked then TBT_AnnounceCB:SetChecked(1) end
...
local restrictedChatType = {
	["SAY"] = true,
	["YELL"] = true
}

local function SendChat(msg, name)
	local channel = TradeLog_FindAnnounceChannel(TradeLog_DB.AnnounceChannel);
	if restrictedChatType[channel] and not IsInInstance() then return end
...
function TradeLog_Output(trade, func, plain)
	...
	if (trade.result == "complete") then
		local playerList = TradeLog_GetTradeList(trade.playerMoney, trade.playerItems, trade.targetItems[7],
			plain and TradeLog_GetMoneyPlainText or TradeLog_GetMoneyColorText)
		local targetList = TradeLog_GetTradeList(trade.targetMoney, trade.targetItems, trade.playerItems[7],
			plain and TradeLog_GetMoneyPlainText or TradeLog_GetMoneyColorText)
		...
		if (#playerList == 0 and #targetList == 0) then
			...
			if (plain) then
				...
				local detailString = TradeLogFrame and
					("|CFF00B4FF|Htradelog:" .. curr().id .. "|h[" .. TRADE_LOG_DETAIL .. "]|h|r") or TRADE_LOG_DETAIL;
...
function TradeLog_OutputLog()
	...
	if (not curr().who) then
		...
		if (curr().reason == "selfhideui") then
			DEFAULT_CHAT_FRAME:AddMessage(string.gsub(TRADE_LOG_FAILED_NO_TARGET, "%%r",
				CANCEL_REASON_TEXT[curr().reason]));
		else
...
function TradeLog_GetMoneyColorText(money)
	...
	if (s > 0) then
		text = text .. "|cffffff00" .. s .. "|r" .. "|cff" .. GSC_SILVER .. TRADE_LOG_MONEY_NAME.silver ..
			"|r";
	end
	if (c > 0) then
		text = text .. "|cffffff00" .. c .. "|r" .. "|cff" .. GSC_COPPER .. TRADE_LOG_MONEY_NAME.copper ..
			"|r";
	end
...
function TradeLog_TradeTooltip(self, trade)
	...
	elseif (trade.result == "cancelled") then
		...
		GameTooltip:AddLine(CANCEL_REASON_TEXT[trade.reason], NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g,
...
function TBT_AnnounceChannelDropDown_OnClick(self)
	...
	TradeLog_DB.AnnounceChannel = self.value;
end

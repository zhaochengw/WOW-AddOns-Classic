...
TBT_PORTAL_MAPPING = {
	[TBT_PORTAL_1] = "Interface\\AddOns\\" .. addonName .. "\\icons\\9.tga",
	[TBT_PORTAL_2] = "Interface\\AddOns\\" .. addonName .. "\\icons\\11.tga",
	[TBT_PORTAL_3] = "Interface\\AddOns\\" .. addonName .. "\\icons\\4.tga",
	[TBT_PORTAL_4] = "Interface\\AddOns\\" .. addonName .. "\\icons\\5.tga",
	[TBT_PORTAL_5] = "Interface\\AddOns\\" .. addonName .. "\\icons\\6.tga",
	[TBT_PORTAL_6] = "Interface\\AddOns\\" .. addonName .. "\\icons\\8.tga",
	[TBT_PORTAL_7] = "Interface\\AddOns\\" .. addonName .. "\\icons\\10.tga",
	[TBT_PORTAL_8] = "Interface\\AddOns\\" .. addonName .. "\\icons\\13.tga",
	[TBT_PORTAL_9] = "Interface\\AddOns\\" .. addonName .. "\\icons\\3.tga",
	[TBT_PORTAL_10] = "Interface\\AddOns\\" .. addonName .. "\\icons\\12.tga",
	[TBT_PORTAL_11] = "Interface\\AddOns\\" .. addonName .. "\\icons\\7.tga",
	[TBT_PORTAL_12] = "Interface\\AddOns\\" .. addonName .. "\\icons\\2.tga",
}
...
function TBTFrame_OnLoad(self)
	...
	hooksecurefunc("TradeFrame_OnShow",
		function(self)
			if (UnitExists("NPC")) then
				TradeFrameTargetInfoText:SetText(UnitClass("NPC") ..
					" - " .. UnitLevel("NPC"));
			end
		end);
	...
	hooksecurefunc("ContainerFrameItemButton_OnModifiedClick", function(self, button)
		...
		if (button == "LeftButton" and IsShiftKeyDown() and AuctionFrame and AuctionFrame:IsVisible()) then
			...
			ChatEdit_InsertLink(C_Container.GetContainerItemLink(self:GetParent():GetID(), self:GetID()));
...
function TBTFrame_CreateLeftButton(class)
	if (getglobal("TradeFramePlayerSpell1Button") ~= nil) then return end
	; --already created
...
-- 造餐术
local function AddMageFood()
	local ConjureRefreshment = GetSpellInfo(42955)
	for _, type in ipairs({ "water", "food" }) do
		local spellTable = TBT_SPELL_TABLE[type]
		tinsert(spellTable, { name = ConjureRefreshment, level = 74, item = 43518 })
		tinsert(spellTable, { name = ConjureRefreshment, level = 80, item = 43523 })
	end
end
...
function TBTFrame_OnPlayerEnter()
	...
	if TBT_MaxSpellRank == nil or not next(TBT_MaxSpellRank) then
		TBT_MaxSpellRank = {};
		if (class == "MAGE") then
			TBT_MagePortals = {}
			AddMageFood()
		...
		while true do
			spellName, spellIcon, spellId = nil, nil, nil
			local function f()
				spellName, _, spellIcon, _, _, _, spellId = GetSpellInfo(i, BOOKTYPE_SPELL)
			end
			pcall(f);
			local spellRankNum;
			local spellSubtext = GetSpellSubtext(spellId)
			if spellSubtext and spellSubtext ~= "" then
				spellRankNum = tonumber(string.match(spellSubtext, TBT_SPELL_RANK_PATTERN))
			end
...
function TBTFrame_OnEvent(self, event, ...)
	...
	if event == "TRADE_SHOW" then
		if TBT_MaxSpellRank and not next(TBT_MaxSpellRank) then
			TBTFrame_OnPlayerEnter()
		end
	end
end
...
function TBT_FindItem(item, quantity, itemType)
	for bag = 0, NUM_CONTAINER_FRAMES do
		for slot = 1, C_Container.GetContainerNumSlots(bag) do
			local info = C_Container.GetContainerItemInfo(bag, slot)
			if info then
				local count, locked = info.stackCount, info.isLocked
				if (count and not locked and count >= quantity) then
					local itemLink = C_Container.GetContainerItemLink(bag, slot);
					local _, _, name = string.find(itemLink, "^|%x+|H.+|h%[(.+)%]")
					local itemID = C_Container.GetContainerItemID(bag, slot)
					if (type(item) == "number" and item == itemID) or (type(item) == "string" and item == name) then
						if (itemType == "stone") then
							...
							for i = 1, GameTooltip:NumLines(), 1 do
								...
								if (text) then
									local _, _, playerName = string.find(text, TBT_GAMETOOLTIP_MADE_BY);
									if (playerName) then
										TBT_debug("name=", playerName);

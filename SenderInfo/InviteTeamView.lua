...
local AutoOpenOnMessage = true;

SenderInfo_ChatPopup = nil

local function CloseRewPopup()
    if SenderInfo_ChatPopup and SenderInfo_ChatPopup:IsShown() then
        SenderInfo_ChatPopup:Hide()
        SenderInfo_ChatPopup:SetParent(nil)
        SenderInfo_ChatPopup = nil
    end
end

local function HandleCombatLessen()
...
--滚动的具体实体
local function CreateScrollItem(name, info, rew)
    local item = CreateFrame("Frame")

    local showWcl = __private.View.Cfg.ShowWCL;
    local useOfficialWCL = __private.View.Cfg.UseOfficialWCL;
    local useUnofficialWCL = __private.View.Cfg.UseUnofficialWCL;
    local heightSize = 30;
    if showWcl then
    ...
    item:SetSize(scrollChild:GetWidth(), heightSize);
    local tex = item:CreateTexture(nil, "BACKGROUND")
    tex:SetAllPoints()
    ...
    local btnChat = CreateFrame("Button", nil, item, "UIPanelButtonTemplate")
    btnChat:SetPoint("RIGHT", -30, 0);
    btnChat:SetSize(28, 28);
    btnChat:SetText("聊");
    btnChat:SetScript("OnClick", function()
        ...
        local msg = rew or "无原始消息记录"
        local popup = CreateFrame("Frame", nil, UIParent, "BackdropTemplate")
        popup:SetSize(600, 150)
        local screenWidth = GetScreenWidth()
        local screenHeight = GetScreenHeight()
        local popupX = (screenWidth - 600) / 2
        local popupY = -(screenHeight + 300) / 2
        popup:SetPoint("TOPLEFT", popupX, popupY)
        popup:SetBackdrop({ bgFile = "Interface/Tooltips/UI-Tooltip-Background", edgeFile =
        "Interface/Tooltips/UI-Tooltip-Border", tile = true, tileSize = 16, edgeSize = 16, insets = { left = 4, right = 4, top = 4, bottom = 4 } })
        popup:SetBackdropColor(0, 0, 0, 0.8)

        popup:EnableMouse(true)
        popup:SetMovable(true)

        local popupMoving = false
        local popupOffsetX, popupOffsetY

        popup:SetScript("OnMouseDown", function(self, button)
            if button == "LeftButton" then
                popupMoving = true
                local mx, my = GetCursorPosition()
                local _, _, _, x, y = popup:GetPoint()
                popupOffsetX = mx - x
                popupOffsetY = my - y
            end
        end)
        ...
        popup:SetScript("OnUpdate", function(self)
            if popupMoving then
                local x, y = GetCursorPosition()
                local nx, ny = x - popupOffsetX, y - popupOffsetY
                popup:SetPoint("TOPLEFT", nx, ny)
            end
        end)

        local meta = popup:CreateFontString(nil, "ARTWORK", "GameFontNormal")
        meta:SetPoint("TOPLEFT", 16, -10)
        meta:SetWidth(550)
        meta:SetJustifyH("LEFT")
        meta:SetText(string.format("原始信息: %s", name or ""))
        ...
        SenderInfo_ChatPopup = popup
    end)

    local btn3 = CreateFrame("Button", nil, item, "UIPanelButtonTemplate")
    btn3:SetPoint("RIGHT", -60, 0);
    btn3:SetSize(28, 28);
    btn3:SetText("查");
    btn3:SetScript("OnClick", function()
...
function InviteTeamView:Add(name, info, rew)
    ...
    if needOpenPanel or AutoOpenOnMessage then
        InviteTeamView:ChangeViewHelper(true);
    end
    ...
    if panel and panel:IsShown() then
        ...
        local item = CreateScrollItem(name, info, rew);
...
function InviteTeamView:Init()
    ...
    AutoOpenOnMessage = __private.View.Cfg.AutoOpenOnMessage
...
combatFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "PLAYER_REGEN_ENABLED" then
        ...
        if CombatLessen and CombatEndViewRecover then
            lastLessenTime = GetTime();
            if panel2 and panelLessen then
                panel2:Show();
                if lessen then
                    lessen:SetText("SenderInfo 缩小");
                end
...
function InviteTeamView:ChangeAutoOpenOnMessage(value)
    AutoOpenOnMessage = value;
end

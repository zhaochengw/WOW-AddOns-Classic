...
local function printSpecLine(tooltip, slot, class_name, spec_name)
    ...
    if (slot_name == "副手" or slot_name == "武器") then
...
local function printClassName(tooltip, class_name)
    --if (class_name:upper() == player_class) then
    --    LibExtraTip:AddLine(tooltip, class_name, 0.074, 0.964, 0.129, false)
    --else
...
local function OnGameTooltipSetItem(tooltip)
    ...
    if (#item > 0) then
        LibExtraTip:AddDoubleLine(tooltip, "阶段名称", "PR/T4/T5/T6/ZA/SWP", 1, 1, 0, 1, 1, 0, false)
    end
    ...
    if not (#item == specs_count) then
        ...
        LibExtraTip:AddLine(tooltip, "按住 alt 以禁用规范过滤。（显示全部职业BIS）", 0.6, 0.6, 0.6, false)
    end
end

...
local igMainMenuOptionCheckBoxOn = 856
local igMainMenuOptionCheckBoxOff = 857
local igMiniMapZoomOut = 824
local igMiniMapZoomIn = 823
...
function srti.UpdateSaved()
	...
	if SRTISaved.thirdPartyFrames and next(SRTISaved.thirdPartyFrames) then
...
function srti.RegisterExternalFrame(frameName, addon, prebuilt)
	...
	if not frameName or not _G[frameName] or not _G[frameName]:IsMouseEnabled() then return end
	if tContains(clickFrames,frameName) then return end
	if (not prebuilt) and (not UnitExists("mouseover")) then return end
	SRTIExternalUF = SRTIExternalUF or {}
	local found
	for addon, frames in pairs(SRTIExternalUF) do

...
local function UIDropDownMenu_StartCountingFix(self)
    if self==DropDownList1 then
        if (FriendsMenuXP:IsVisible()) then
            _G[FriendsMenuXP:GetName() .. "Highlight"]:Hide()
        elseif (FriendsMenuXPSecure:IsVisible()) then
            FriendsMenuXPSecure:Hide()
            --_G[FriendsMenuXPSecure:GetName() .. "Highlight"]:Hide()
        end
    end
end

local function UIDropDownMenu_StopCountingFix(self)
    if self==DropDownList1 then
        if (FriendsMenuXP:IsVisible()) then
            _G[FriendsMenuXP:GetName() .. "Highlight"]:Show()
        elseif (FriendsMenuXPSecure:IsVisible()) then
            FriendsMenuXPSecure:Hide()
            --_G[FriendsMenuXPSecure:GetName() .. "Highlight"]:Show()
        end
    end
end
...
function FriendsMenuXP_OnLoad(self)
    ...
    SetOrHookScript(DropDownList1, "OnHide", function()
        FriendsMenuXP:Hide();
        if(not InCombatLockdown()) then FriendsMenuXPSecure:Hide(); end
        UIDropDownMenu_StopCountingFix(FriendsMenuXP)
        UIDropDownMenu_StopCountingFix(FriendsMenuXPSecure)
    end)

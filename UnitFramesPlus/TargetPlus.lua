...
function UnitFramesPlus_TargetHPValueDisplayUpdate()
    ...
    local CurHPfix, MaxHPfix, LossHPfix;
    -- 根据 hpmpunit 决定是否进行进位处理
    if UnitFramesPlusDB["target"]["hpmpunit"] == 1 then
        -- 启用进位时，根据 unittype 选择进位方式
        CurHPfix, MaxHPfix, LossHPfix = UnitFramesPlus_GetValueFix(CurHP, MaxHP, UnitFramesPlusDB["target"]["hpmpunit"], UnitFramesPlusDB["target"]["unittype"]);
    else
        -- 不启用进位时显示原始数值
        CurHPfix = CurHP;
        MaxHPfix = MaxHP;
        LossHPfix = MaxHP - CurHP;
    end
...
function UnitFramesPlus_TargetMPValueDisplayUpdate()
    ...
    local CurMPfix, MaxMPfix, LossMPfix;
    -- 根据 hpmpunit 决定是否进行进位处理
    if UnitFramesPlusDB["target"]["hpmpunit"] == 1 then
        -- 启用进位时，根据 unittype 选择进位方式
        CurMPfix, MaxMPfix, LossMPfix = UnitFramesPlus_GetValueFix(CurMP, MaxMP, UnitFramesPlusDB["target"]["hpmpunit"], UnitFramesPlusDB["target"]["unittype"]);
    else
        -- 不启用进位时显示原始数值
        CurMPfix = CurMP;
        MaxMPfix = MaxMP;
        LossMPfix = MaxMP - CurMP;
    end
...
function UnitFramesPlus_TargetExtraTextFontSize()
    ...
    local BarFontSize = tonumber(UnitFramesPlusDB["target"]["fontsize"]) or 12;
    local outline = nil;
    local outlineValue = tonumber(UnitFramesPlusDB["target"]["outline"]) or 0;

    if outlineValue == 1 then
        outline = "OUTLINE";
    elseif outlineValue == 2 then
    ...
    if not IsAddOnLoaded("EasyFrames") then
        TargetFrameTextureFrameName:SetFont(STANDARD_TEXT_FONT, BarFontSize, outline);
        TargetFrameHealthBar.RightText:SetFont(BarFont, BarFontSize, outline);
        TargetFrameHealthBar.LeftText:SetFont(BarFont, BarFontSize, outline);
        TargetFrameHealthBar.TextString:SetFont(BarFont, BarFontSize, outline);
        TargetFrameManaBar.RightText:SetFont(BarFont, BarFontSize, outline);
        TargetFrameManaBar.LeftText:SetFont(BarFont, BarFontSize, outline);
        TargetFrameManaBar.TextString:SetFont(BarFont, BarFontSize, outline);
    end
end

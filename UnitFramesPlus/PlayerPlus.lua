...
local function UnitFramesPlus_PlayerShiftDrag()
    ...
    PlayerFrame:SetScript("OnMouseUp", function(self, elapsed)
        if UnitFramesPlusVar["player"]["moving"] == 1 then
            ...
            -- 添加位置值验证，防止异常值导致框体位置异常
            if left and bottom then
                -- 限制位置在合理范围内，防止异常值
                left = math.max(-1000, math.min(left, 2000));
                bottom = math.max(-1000, math.min(bottom, 2000));
...
function UnitFramesPlus_PlayerPositionSet()
    if UnitFramesPlusVar["player"]["moved"] == 0 then
        ...
        -- 获取保存的位置值并验证
        local x = UnitFramesPlusVar["player"]["x"] or 0;
        local y = UnitFramesPlusVar["player"]["y"] or 0;
        
        -- 验证位置值是否合理，防止异常值导致框体位置异常
        x = math.max(-1000, math.min(x, 2000));
        y = math.max(-1000, math.min(y, 2000));
        
        PlayerFrame:ClearAllPoints();
        PlayerFrame:SetPoint("BOTTOMLEFT", UIParent, "BOTTOMLEFT", x, y);
    end
end
...
function UnitFramesPlus_PlayerDragon()
    ...
    -- 使用原始尺寸而非完全跟随父框体，防止材质异常拉伸
    PlayerFrameTexture:SetWidth(PlayerFrame:GetWidth());
    PlayerFrameTexture:SetHeight(PlayerFrame:GetHeight());
    PlayerFrameTexture:ClearAllPoints();
    PlayerFrameTexture:SetPoint("CENTER", PlayerFrame, "CENTER");

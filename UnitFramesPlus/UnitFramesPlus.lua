...
local function UnitFramesPlus_Options_Init()
    ...
    -- 改为使用安全的方式检测
    local function CheckRaidFrames()
    ...
    C_Timer.After(1, CheckRaidFrames);
end
...
local function UnitFramesPlus_Init()
    ...
    local function InitializeRaidFrames()
        if InCombatLockdown() then 
            C_Timer.After(1, InitializeRaidFrames)
            return 
        end
    ...
    -- 延迟执行团队框架初始化
    C_Timer.After(0.5, InitializeRaidFrames)
end

local function UnitFramesPlus_CVar()
    -- 添加安全的CVar设置
    local function SetPartyCVars()
        if InCombatLockdown() then 
            C_Timer.After(1, SetPartyCVars)
            return 
        end
    ...
    -- 延迟执行团队相关CVar设置
    C_Timer.After(0.5, SetPartyCVars)
end
...
function UnitFramesPlus_GetValueFix(valueCurr, valueMax, valueunit, unittype)
    ...
    elseif valueunit == 1 and unittype == 2 and (GetLocale() == "zhCN" or GetLocale() == "zhTW") then
        ...
        elseif valueLoss >= 10000 then    -- 修复：将 valueMax 改为 valueLoss
        ...
        elseif valueLoss >= 1000 then    -- 修复：将 valueMax 改为 valueLoss

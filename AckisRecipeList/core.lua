...
function addon:OnInitialize()
	...
	-------------------------------------------------------------------------------
	local scan_button = CreateFrame("Button", "ARL_ScanButton", UIParent, "UIPanelButtonTemplate, BackdropTemplate")
    if not scan_button then
        self:Print("Failed to create scan button!")
        return
    end
    scan_button:SetHeight(24)  -- 增加高度适应新UI
    
    -- 设置按钮文本前确保模板加载完成
    C_Timer.After(0.1, function()
        if scan_button:GetObjectType() == "Button" then
            scan_button:SetText(L["Scan"])
        end
    end)

    -- 显式设置父框架防止空指针
    scan_button:SetParent(UIParent)
	...
	function addon:Scan(textdump, is_refresh)
		...
		if MRTUIUtils_PushFilterSelection then
			...
			if TradeSkillInvSlotDropDown then
				...
				addon:Debug("TradeSkillInvSlotDropDown is nil!")
			end
			...
			if TradeSkillSubClassDropDown then
				...
				addon:Debug("TradeSkillSubClassDropDown is nil!")
			end

...
ns.PROJECT_DATA = { --
    ...
    [15] = {maxLevel = 85, name = EXPANSION_NAME3, projects = {15, 11, 5, 2}},
    [19] = {maxLevel = 90, name = EXPANSION_NAME4, projects = {19, 15, 11, 5, 2}},
}
...
function ns.DataMake(allowCrossRealm)
    ...
    local function Realm(realm)
        realm = decode(realm)
        local realmNum = tonumber(realm)
        if allowCrossRealm or realmNum == GetRealmID() then
            currentRealm = GetRealmID()
            return
        end
        currentRealm = nil
    end
    ...
    local function Name(name)
        if not currentRealm or currentRealm ~= GetRealmID() then
            return nop
        end

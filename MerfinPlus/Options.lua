...
do
  if IsTBC() then
    local function SetDefaultProfiles()
      if MerfinPlus.defaults.global.wowSims.defaults then
        return
      end
      local profiles = MerfinPlus.defaults.global.wowSims.profiles
      ...
      MerfinPlus.defaults.global.wowSims.defaults = true
    end
    SetDefaultProfiles()
    local function AutoAssignWoWSimsProfileIfMissing()
      if not MerfinPlusSaved or not MerfinPlusSaved.global then
        return
      end

      MerfinPlusSaved.global.wowSims = MerfinPlusSaved.global.wowSims or {}
      local wowSims = MerfinPlusSaved.global.wowSims
      ...
      for key, profile in pairs(profiles) do
        if profile and profile.class == class and profile.specID == specID and key and key:match("PreRaidBIS$") then

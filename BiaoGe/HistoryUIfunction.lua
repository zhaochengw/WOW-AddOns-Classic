...
------------------底色材质------------------
function BG.HistoryDiSsUI(FB,t,b,bb,i,ii)
    local TextureDs ={}
        -- 先做底色材质1（鼠标悬停的）
    local ds = CreateFrame("Button", nil, BG["HistoryFrame"..FB])
    ds:SetSize(331, 18)
    ...
    BG.HistoryFrameDs[FB..1]["boss"..BossNum(FB,b,t)]["ds"..i] = ds
end
...
function BG.HistoryBossNameUI(FB,t,b,bb,i,ii)
    local version = BG["HistoryFrame"..FB]:CreateFontString();
    version:SetPoint("TOP", BG.HistoryFrame[FB]["boss"..BossNum(FB,b,t)].zhuangbei1, "TOPLEFT", -45, -3)           
    version:SetFontObject(GameFontNormal)
    version:SetText(BiaoGe[FB]["boss"..BossNum(FB,b,t)].bossname)                
    version:Show()
    BG.HistoryFrame[FB]["boss"..BossNum(FB,b,t)]["bossname"] = version
    if BG.HistoryFrame[FB]["boss"..BossNum(FB,b,t)] == BG.HistoryFrame[FB]["boss"..Maxb[FB]+2] then
        local version1 = BG["HistoryFrame"..FB]:CreateFontString()
        version1:SetPoint("BOTTOM", BG.HistoryFrame[FB]["boss"..Maxb[FB]+2].zhuangbei5, "BOTTOMLEFT", -45, 7)

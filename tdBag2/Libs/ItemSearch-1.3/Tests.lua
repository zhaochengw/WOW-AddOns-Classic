local Tests = WoWUnit and WoWUnit('ItemSearch-1.3', 'PLAYER_LOGIN', 'GET_ITEM_INFO_RECEIVED')
...
function Tests:BasicItem()
	local item = '\124cffa335ee\124Hitem:12590::::::::60:::::\124h[Felstriker]\124h\124r'

	IsTrue(Search:Matches(item, 'felstriker'))
	IsTrue(Search:Matches(item, 'dagger'))
	IsTrue(Search:Matches(item, 'slot:hand'))

	IsTrue(Search:Compile('felstriker')(item))
	IsTrue(Search:Compile('dagger')(item))
	IsTrue(Search:Compile('slot:hand')(item))
end
